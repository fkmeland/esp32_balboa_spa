...
static const char kPortalStatusJs[] PROGMEM =
    ...
    "function statusSetButtonState(code,desired,pumpRaw){var btns=document.querySelectorAll('button[data-button=\"'+code+'\"]');if(!btns||btns.length===0)return;"
    "if(btns.length===1){btns[0].setAttribute('data-state',desired);btns[0].textContent='Turn '+(desired==='on'?'On':'Off');}"
    "else if(typeof pumpRaw!=='undefined'){for(var i=0;i<btns.length;i++){var st=btns[i].getAttribute('data-state');btns[i].disabled=(st==='off'&&pumpRaw===0)||(st==='low'&&pumpRaw===1)||(st==='high'&&pumpRaw===2);}}"
    "}"
    ...
    "statusSetButtonState(4,snap.pump1On?'off':'on',Number(snap.pump1||0));"
    "statusSetButtonState(5,snap.pump2On?'off':'on',Number(snap.pump2||0));"
    "statusSetButtonState(6,snap.pump3On?'off':'on',Number(snap.pump3||0));"
    "statusSetButtonState(7,snap.pump4On?'off':'on',Number(snap.pump4||0));"
    "statusSetButtonState(8,snap.pump5On?'off':'on',Number(snap.pump5||0));"
    "statusSetButtonState(9,snap.pump6On?'off':'on',Number(snap.pump6||0));"
    ...
    "statusStartPolling();"
    "})();"
    "function statusButtonMatch(snap,code,desired){var ds=(desired||'on').toLowerCase();"
    "var on=(ds==='on'||ds==='high'||ds==='low');"
    "if(code===17)return (snap.light1>0)===on;"
    "if(code===18)return (snap.light2>0)===on;"
    "var pm=function(v){if(ds==='off')return v===0;if(ds==='low')return v===1;if(ds==='high')return v===2;return v>0;};"
    "if(code===4)return pm(snap.pump1);"
    "if(code===5)return pm(snap.pump2);"
    "if(code===6)return pm(snap.pump3);"
    "if(code===7)return pm(snap.pump4);"
    "if(code===8)return pm(snap.pump5);"
    "if(code===9)return pm(snap.pump6);"
